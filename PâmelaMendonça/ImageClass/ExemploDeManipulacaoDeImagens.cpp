...
typedef struct // Cria uma STRUCT para armazenar o RGB
{
    unsigned char red;
    unsigned char green;
    unsigned char blue;
...
void init()
{
    ...
    r = Image.Load("Mickey.png"); // Carrega uma imagem
...
void Fill8(int x,int y) //8 vizinhos
{
    ...
    {
        //pita
        Image.DrawPixel(x,y,NEW_COLOR.red, NEW_COLOR.green, NEW_COLOR.blue);//Image(x,y) = NEW_COLOR;
...
void Fill4(int x,int y) //4 vizinhos
{
    if ((Image.ReadR(x,y) == OLD_COLOR.red)&&(Image.ReadG(x,y) == OLD_COLOR.green)&&(Image.ReadB(x,y) == (int)OLD_COLOR.blue)) //se o pixel é da mesma cor do antigo seed
    {
        //pita
...
void Preenche(int Seed_x,int Seed_y,unsigned char Novo_r,unsigned char Novo_g,unsigned char Novo_b)
{
    ...
    //atribui a cor nova
    NEW_COLOR.red = Novo_r;
    NEW_COLOR.green = Novo_g;
